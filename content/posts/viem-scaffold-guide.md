...
根据 2024 年的开发者调查，Viem 凭借其出色的 TypeScript 支持和轻量级设计，正在成为 Web3 开发的新趋势。相比 wagmi 等高级抽象库，纯 Viem 方案具有以下优势：

1. **体积小**：只装一个 `viem`，比 wagmi 全家桶少 70%+ 的包
2. **零魔法**：每一行代码你都知道它到底在干什么
3. **调试友好**：出错直接看 Viem 抛的错误，不用猜是哪个 hook 的问题
4. **未来证明**：wagmi v2 底层已经完全换成 Viem
